...
function update_θ_residual(theta::CuDeviceArray{R}, theta_0::CuDeviceArray{R}, residual::CuDeviceArray{R}, c::CuDeviceArray{R}, eta_mu_i::CuDeviceArray{R}, eta_mu::R, adjust::Bool, minv::R, maxv::R) where R
    N = size(theta, 1)
    tid = threadIdx().x + blockDim().x * (blockIdx().x - 1)
    if tid > N
        return
    ...
    @inbounds begin
        difference = 2(residual[tid] - c[tid]) / eta_mu_i[tid]
        thetav = theta_0[tid]
    end
...
function extragradient_ot_dual(r::CuArray{R},
    ...
    for i in 1:args.itermax
        ...
        infeas(ν, ηt, st)
        @cuda threads = threads blocks = linear_blocks update_θ_residual(θ̄, θ, residual_storage, c, eta_mu, args.eta_mu, false, minv, maxv)
        ...
        @cuda threads = threads blocks = linear_blocks update_θ_residual(θ, θ, residual_storage, c, eta_mu, args.eta_mu, true, minv, maxv)
...
function papc_eot_dual(r::AbstractArray{R},
    ...
    for i in 1:args.itermax
        ...
        infeas(μ⁺pa, μ⁻pa)
        # eta_mu = (residual_storage) ./ (args.C2)
        # st = st + ηp * sumval(μ⁺pa, μ⁻pa)
        # residual_sum += (residual_storage - c)
        ...
        μ⁻ ./= normv
        μ⁺ ./= normv
        # mup_temp = max.(μ⁺, exp(-args.B) * max.(μ⁺, μ⁻))
        # mun_temp = max.(μ⁻, exp(-args.B) * max.(μ⁺, μ⁻))
        # normv = (mup_temp + mun_temp)
        # μ⁻ .= mun_temp ./ normv
        # μ⁺ .= mup_temp ./ normv
...
# 3.24004515406566e-03
        ...
        while true
            st = (1 - ηp / L) * st + ηp / L
            dual_bar = dualv(μ⁺pa, μ⁻pa, st, W, W∞, ηp, r, c)
            argument =  (residual_storage - c) ./ eta_mu / L
            maxval = max.(argument, -argument)
            μ⁻t = μ⁻ .* exp.(-argument .- maxval)#(sum(r .* pt, dims=1)' - c))
            μ⁺t = μ⁺ .* exp.(argument .- maxval)
            normv = (μ⁻t + μ⁺t)
            μ⁻t ./= normv
            μ⁺t ./= normv
            dual_curr = dualv(μ⁺t, μ⁻t, st, W, W∞, ηp, r, c)
            niter += 1
            if dual_curr - dual_bar - dot(-(residual_storage - c), μ⁺t-μ⁺pa-(μ⁻t-μ⁻pa)) <= L * dual_kl(μ⁺t, μ⁺pa, eta_mu)
                μ⁺ .= μ⁺t
                μ⁻ .= μ⁻t
                μ⁻p = μ⁻p + ηp * (μ⁻ - μ⁻p)
                μ⁺p = μ⁺p + ηp * (μ⁺ - μ⁺p)
        ...
        # argument =  (residual_storage - c) ./ eta_mu
        # maxval = max.(argument, -argument)
        # μ⁻ = μ⁻ .* exp.(-argument .- maxval)#(sum(r .* pt, dims=1)' - c))
        # μ⁺ = μ⁺ .* exp.(argument .- maxval)
        # normv = (μ⁻ + μ⁺)
