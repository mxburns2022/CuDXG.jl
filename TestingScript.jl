...
args = EOTArgs(B=1.0, eta_p=1e-2, eta_mu=0.0, itermax=1000, verbose=true, epsilon=1e-10)
...
function test_gradient()
    ...
    ψ = rand(N)
    W = rand(N, N)
    args = EOTArgs(B=1.0, eta_p=1e-5, eta_mu=0.0, itermax=50000, verbose=true)
...
function test_formulation()
    ...
    b = rand(rng, 5)
    W = rand(rng, 5, 5)
    r = sofitermax(W .+ a .+ b', dims=2; normalize_values=false)
    c = sofitermax((W .+ a .+ b')', dims=2; normalize_values=false)
