...
sizes = [64]
if ARGS[1] == "1"
    instance_pairs = [(5, 9), (2, 6), (4, 6), (6, 10), (5, 7)]
else
    instance_pairs = [(3, 8), (2, 3), (1, 9), (7, 8), (2, 4)]
end
algorithms = [keys(solvers)...]
pvals = ["1", "2", "Inf"]
# pvals = ["Inf"]
algorithms = [("annealed_sinkhorn", 1e-1), ("sinkhorn", 1e-4), ("sinkhorn", 1e-5), ("lamp", 0.0)]
# algorithms = [("annealed_sinkhorn", 1e-2)]
...
for (p, (solver,eps), (ind1, ind2),  probclass, size) in product(pvals, algorithms, instance_pairs, benchmark_classes, sizes)
    ...
    println(joinpath(output_directory, output_file), " ", isfile(joinpath(output_directory, output_file)))
